...
// -----------------------------------//
let n = prompt("Nhập id của user bạn muốn tìm:")
n = Number(n);
...
// Nhập từ bàn phím vào 1 chuỗi keyword
...
let countAgeOver50 = users.filter(user => user.age > 50).length;
console.log("Số lượng user có age > 50: " + countAgeOver50);

// Đếm số lượng user đã kết hôn và chưa kết hôn
let countMarried = users.filter(user => user.is_married).length;
let countUnmarried = users.length - countMarried;
console.log("Số lượng user đã kết hôn: " + countMarried);
console.log("Số lượng user chưa kết hôn: " + countUnmarried);

// Đếm số lượng user theo từng ngành nghề tương ứng
let jobCounts = users.reduce((counts, user) => {
    counts[user.job] = (counts[user.job] || 0) + 1;
    return counts;
}, {});
console.log("Số lượng user theo từng ngành nghề: ", jobCounts);